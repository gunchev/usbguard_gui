...
6. Increase coverage for device_dialog and screensaver (more qtbot tests or pure logic extraction).
   Measured 2026-09-13: `device_dialog` **95%** (6 lines missed: 61, 141-145), `screensaver` **83%**
   (39 missed, mostly Qt event-loop / D-Bus-callback paths). Not carried in TODO.md's 1.0 list —
   open-ended unless a numeric target is set, so treat the numbers above as the current state rather
...
8. Before releases: always `make check` + verify wheel contains the `introspection/*.xml` files.
   Re-verified 2026-09-13: the 0.7.4 wheel ships all four XMLs (`org.usbguard.Devices1`,
   `org.usbguard.Policy1`, `org.freedesktop.ScreenSaver`, `org.freedesktop.DBus`) — hatchling
   includes package files by default, so no `MANIFEST.in` is needed. Standing pre-release instruction,
   not a one-off task.
