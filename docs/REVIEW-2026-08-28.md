...
- ✅ dbus-fast 5.0.22 (installed in the dev venv, far newer than the `>=1.0` pin) is
  ...
  **VERIFIED 2026-09-13** against the live daemon on `dg-lp2` (usbguard 1.1.4,
  `busctl introspect org.usbguard1 /org/usbguard1/Devices`): `DevicePresenceChanged uuusa{ss}`,
  `DevicePolicyChanged uuusua{ss}`, `applyDevicePolicy uub → u` and `listDevices s → a(us)` all
  match the bundled XML exactly. Nothing left to re-check here.
...
- ✅ CI installs `python3.10…3.13` but `tox.ini` envlist includes `py314` — silently
  skipped via `skip_missing_interpreters`; either add the package or drop the env.
  **RESOLVED 2026-09-13**: `.github/workflows/fedora.yml` now installs `3.10`–`3.14` (altver loop),
  and the current master runs execute all seven envs — `lint`, `typecheck`, `py310`–`py314`. None skipped.
